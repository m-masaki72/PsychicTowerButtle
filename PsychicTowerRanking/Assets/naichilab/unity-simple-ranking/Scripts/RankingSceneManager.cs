...
namespace naichilab
{
	public class RankingSceneManager : MonoBehaviour
	{
		...
		[SerializeField] WebGLNativeInputField nameInputField;
		...
		private string InputtedNameForSave {
			get {
				return this.nameInputField.textComponent.text;
			}
		}
		...
		IEnumerator GetHighScoreAndRankingBoard ()
		{
			...
			//ハイスコア取得
			{
				...
				if (hiScoreCheck.Count > 0) {
					...
					//this.nameInputField.textComponent.text  = highScoreSpreadSheetObject ["name"].ToString ();
				} else { 
					//登録されていない
